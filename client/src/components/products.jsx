  ...
  return (
    <div className="flex flex-wrap justify-center gap-[50px]">
      {products.map((product) => (
        <div key={product.productID} className="w-80 h-80">
          <div className="border p-4 hover:scale-105 transition-transform bg-opacity-10 bg-white rounded-lg shadow w-80 h-80">
            <img src={product.imageUrl} alt={product.productName} className="w-[250px] h-[250px] object-cover mb-7 ml-[30px]" />
            <div className="text-white">
              <h2 className="text-xl font-bold mb-2">{product.productName}</h2>
              <p className="text-white flex items-stretch">${product.price}</p>
